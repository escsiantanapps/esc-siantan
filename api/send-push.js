...
const VAPID_SUBJECT = process.env.VAPID_SUBJECT || 'mailto:admin@escsiantan.app'
...
export default async function handler(req, res) {
  ...
  if (!SUPABASE_URL || !SERVICE_ROLE_KEY || !VAPID_PUBLIC || !VAPID_PRIVATE) {
    return res.status(500).json({ error: 'Server belum dikonfigurasi (env VAPID/Supabase belum lengkap).' })
  }
