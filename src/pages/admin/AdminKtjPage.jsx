...
export default function AdminKtjPage() {
  ...
  useEffect(() => {
    setLoading(true)
    registrationService.getAllKtj({ status })
  ...
  return (
    <div>
      ...
      {!loading && registrations.length === 0 && (
        <EmptyState icon={CreditCard} title={t('aktj.empty')} description={t('areg.emptyDesc')} />
      )}

      {!loading && registrations.length > 0 && (
        <Card className="divide-y divide-gray-100">
          {registrations.map(r => (
            <Link key={r.ktj_id} to={`/admin/ktj/${r.ktj_id}`} className="flex items-center gap-3 p-3.5 hover:bg-gray-50 transition-colors">
